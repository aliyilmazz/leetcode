...
class Codec {
    ...
    TreeNode dummyNode = new TreeNode(10000);
    ...
    public String serialize(TreeNode root) {
        ...
        nodeQueue.add(root);
        nodeList.add(Integer.toString(root.val));
        //boolean nextLevelIsClear = false;

        while (!nodeQueue.isEmpty()/* && !nextLevelIsClear */) {
            Queue<TreeNode> newLevelQueue = new LinkedList<>();
            //nextLevelIsClear = true;
            while (!nodeQueue.isEmpty()) {
                TreeNode currentNode = nodeQueue.poll();
                

                //if (newLevelQueue.isEmpty() && currentNode.left == null && currentNode.right == null) continue;

                //if (currentNode.val==10000) { nodeList.add("null"); continue; }

                if (currentNode.left != null) {
                    newLevelQueue.add(currentNode.left);
                    nodeList.add(Integer.toString(currentNode.left.val));
                }
                else {
                         nodeList.add("null");
//                    newLevelQueue.add(dummyNode);
                }
//                else {
//                    newLevelQueue.add(dummyNode);
//                }
                ...
                if (currentNode.right != null) {
                    newLevelQueue.add(currentNode.right);
                    nodeList.add(Integer.toString(currentNode.right.val));
                }
                else {
                    nodeList.add("null");
                }
...
//                }
            ...
            nodeQueue = newLevelQueue;
        }

        int nullRemoverIndex = nodeList.size()-1;
        while(nodeList.get(nullRemoverIndex).equals("null")) { nodeList.remove(nullRemoverIndex--); }
        
        return nodeList.stream().map(i -> {
            if (i.equals("10000")) return "null"; else return i;
    ...
    public TreeNode deserialize(String data) {
        ...
        //Queue<String> inputQueue = Arrays.stream(data.split(",")).collect(Collectors.toCollection(LinkedList::new));

        int inputQueueIterator = 0;
        Queue<TreeNode> decorationQueue = new LinkedList<>();

        TreeNode root = new TreeNode(Integer.parseInt(values[inputQueueIterator++]));
        decorationQueue.add(root);

        while (inputQueueIterator < values.length) {
            Queue<TreeNode> nextLevelDecorationQueue = new LinkedList<>();
            while (inputQueueIterator < values.length && !decorationQueue.isEmpty()) {
                TreeNode currentDecorationNode = decorationQueue.poll();
//                if (currentDecorationNode.val == 10000) {
//                    nextLevelDecorationQueue.add(dummyNode);
//                    nextLevelDecorationQueue.add(dummyNode);
//                    inputQueueIterator+=2;
//                    continue;
//                }
                String leftValue = values[inputQueueIterator++];
                if (leftValue.equals("null")) {} //nextLevelDecorationQueue.add(dummyNode);
                else {
                    currentDecorationNode.left = new TreeNode(Integer.parseInt(leftValue));
                    nextLevelDecorationQueue.add(currentDecorationNode.left);
                }

                if (inputQueueIterator==values.length) break;

                String rightValue = values[inputQueueIterator++];
                if (rightValue.equals("null")) {} // nextLevelDecorationQueue.add(dummyNode);
                else {
                    currentDecorationNode.right = new TreeNode(Integer.parseInt(rightValue));
                    nextLevelDecorationQueue.add(currentDecorationNode.right);
                }
            }
            decorationQueue = nextLevelDecorationQueue;
        }
...
public class SerializeAndDeserializeBinaryTree {
    ...
    public static void main(String[] args) {
        ...
        //TreeNode root = new TreeNode(3, new TreeNode(2, new TreeNode(3), null), new TreeNode(4));
