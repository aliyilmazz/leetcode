...
public class WIP__MaxProfitWithCooldown {

    HashMap<Integer, Integer> arrayHashCodeToTotalProfit;

    public WIP__MaxProfitWithCooldown() {
        this.arrayHashCodeToTotalProfit = new HashMap<>();
    }

    public int _maxProfit(int[] profits, int currentProfitIndex, boolean currentlyBuying) {

        int hashCode = Arrays.hashCode(profits);
        if (this.arrayHashCodeToTotalProfit.containsKey(hashCode)) {
            return this.arrayHashCodeToTotalProfit.get(hashCode);
        }

        // else, compute the result and then store result into HashMap before returning the result
        int totalProfit = 0;
        int nextProfit, currentProfit, previousProfit;
        while (currentProfitIndex < profits.length) {

            currentProfit = profits[currentProfitIndex];
            if (currentProfit > 0) {
            ...
            else { // currentProfit is negative

                if (!currentlyBuying) {
                    // if not currently buying, skip negative profits
                    currentProfitIndex++;
                    continue;
                }

                // now we're currently buying and we face negative profit.
                // let's see
                //      [A] if we can soak next two profits and increase our profit,
                //      or
                //      [B] if we should early-sell, get rid of last profit, and save nextProfit
                //      or
                //      [C] both next two are negative, then just sell and get rid of these negative streaks.

                if (currentProfitIndex < profits.length-1) { // check if we can look ahead (nextProfit)
                    nextProfit = profits[currentProfitIndex+1];

                    previousProfit = profits[currentProfitIndex-1]; // guaranteed to not throw segfault since we're currentlybuying.

                    if (nextProfit <= 0) {
                        // [C] both next two are negative.
                        // just sell, and get rid of these negative streaks
                        currentProfitIndex+=2;
                        currentlyBuying = false;
                    }
                    else {

                        int totalProfitWithSoaking = 0;
                        if (nextProfit + currentProfit > 0) {
                            // a possibility might be soaking.
                            totalProfitWithSoaking = totalProfit + (nextProfit+currentProfit) + _maxProfit(
                                    profits, currentProfitIndex+2, true
                            );
                        }

                        int totalProfitWithoutPreviousProfit = totalProfit - previousProfit + _maxProfit(
                                profits, currentProfitIndex+1, true
                        );
                        int totalProfitWithoutNextProfit = totalProfit + _maxProfit(
                                profits, currentProfitIndex+2, false
                        );

                        return Math.max(
                                Math.max(totalProfitWithoutPreviousProfit, totalProfitWithoutNextProfit),
                                totalProfitWithSoaking
                        );
                    }

//                    if (currentProfit+nextProfit > 0) {
//                        // soak OR earlysell.
//                        int possibleSoakValue = currentProfit + nextProfit;
//
//
//
//                        int earlySellProfitValue = (nextProfit - previousProfit);
//
//                        if (possibleSoakValue>=earlySellProfitValue) {
//                            // [A] soak current + next profit if they are positive together.
//                            totalProfit += possibleSoakValue;
//                            // still continue buying without any buy-sell
//                            // currentlyBuying = true; (its already true so its commented)
//                        }
//                        else {
//                            // [B] early sell and benefit from nextProfit.
//                            totalProfit += earlySellProfitValue;
//                            // still continue buying since we sold at previousProfit, and bought nextProfit.
//                            // currentlyBuying = true; (its already true so its commented)
//                        }
//                        // currentlyBuying = true; (its already true so its commented)
//                        currentProfitIndex += 2;
//                    }
//                    else {
//                        // at this point: we cant soak two ([A] is false) and nextProfit is not negative ([C] is false)
//                        // actions:
//                        // [C1] just sell and ignore these two profits in line.
//                        // [C2] replace previous with next. (same as [B])
//
//                        if (nextProfit>previousProfit) { // [C2]
//                            // apply [B] scenario
//                            totalProfit += (nextProfit-previousProfit);
//                        }
//                        else {
//                            // [C1] no point to replace, just skip those two.
//                            // no action.
//                        }
//                        currentProfitIndex += 2;
//                        currentlyBuying = false;
//                    }
                    ...
                    // we are on a negative value, and we can't look ahead.
                    // action: sell. increment index to return totalProfit in next iteration
                    currentProfitIndex++;
                    currentlyBuying = false;
                }
            }
        }
        this.arrayHashCodeToTotalProfit.put(hashCode, totalProfit);
        return totalProfit;
    }
    ...
    public int maxProfit(int[] prices) {
        ...
        return _maxProfit(profits, 0, false);
    }
//

    public static void main(String[] args) {
        WIP__MaxProfitWithCooldown maxProfitCls = new WIP__MaxProfitWithCooldown();
        int[] prices = new int[]{
        ...
        int maxProfit = maxProfitCls.maxProfit(prices);
